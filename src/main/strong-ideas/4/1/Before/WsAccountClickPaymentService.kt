...
import some.company.api.payment.account.AccountClickPaymentAddResponse
...
class WsAccountClickPaymentService(
    ...
    suspend fun createAuditOperation(
    ...
    ): AccountClickPaymentAddResponse? = try {
        ...
        accountClickPaymentAddClient.async(headers.channelId, request).awaitSingle()
    } catch (e: Exception) {
    ...
    suspend fun changeAuditOperationStatus(
        ...
        if (auditOperationReference == null) {
            logger.error {
                "Can't update CL2 operation for user ${headers.userId} with status $status due to no reference"
            }
            return
        }
